...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query("SELECT e FROM Employee e WHERE e.email=?1 AND e.salary=?2")               //  ?  means it will do with position and we give a number for it
    ...
    @Query("SELECT e FROM Employee e WHERE e.firstName=:name OR e.salary=:salary")          //    :   means it will do with name we define it with @Param
    ...
    @Query(value = "SELECT * FROM employees WHERE salary = ?1",nativeQuery = true )  // here employees is TABLE name
    List<Employee> readEmployeeBySalary(int salary);

    @Modifying
    @Transactional   //  More than one action
