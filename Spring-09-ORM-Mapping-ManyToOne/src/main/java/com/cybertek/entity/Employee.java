...
//    private String department;        //This column will come from JOIN
    ...
    @ManyToOne(cascade = {CascadeType.DETACH, CascadeType.MERGE, CascadeType.PERSIST}) //For taking not all of them use one by one {CascadeType.DETACH, CascadeType.MERGE, CascadeType.PERSIST}
...
//    private int regionId;         //This column will come from JOIN
                          //if we use   CascadeType.ALL here deleting #1 employee will also delete region 1
    @OneToOne(cascade = {CascadeType.DETACH, CascadeType.MERGE, CascadeType.PERSIST}, fetch = FetchType.LAZY) //fetch = FetchType.LAZY do when needed  or else fetch = FetchType.EAGER
